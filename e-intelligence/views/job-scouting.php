                    ...
                    <div class="card-header">
                        ...
                        <div class="btn-actions-pane-right">
                            <div role="group" class="btn-group-sm btn-group">
                                <form action="job-scouting.php?result" method="post">
                    ...
                    <table class="mb-0 table table-dark">                        
                        ...
                        <tbody>
                            ...
                            if (mysqli_num_rows($result_uncofirmed) > 0)
                                ...
                                while ($row = mysqli_fetch_array($result_uncofirmed)){
                                       $name_skil = $row['Name'];
                                       $sqlSelect_u = "SELECT * FROM skill where Name = '$name_skil' and user_id = '$user_id' and employer_id = '$inst_grads'  and scouted = 0 order by rating DESC";                   
                                ...
                                <td>
                                    <form action="../api_controller/employ-skill.php" method="post" >
                                        ...
                                        <input name="user_id" id="id" value="<?php echo $inst_grads; ?>" 
                                               type="hidden" class="form-control">
                                        <button type="submit" id="PopoverCustomT-1" class="btn btn-primary btn-sm" name="apply">
                                            Apply
                                        </button>
                                    </form>
                                </td>
                        ...
                        <tbody>
                            <?php
                                    $sqlSelect_uncofirmed = "SELECT * FROM skill where user_id = '$user_id' and scouted = 0";                   
                                    $result_uncofirmed = mysqli_query($conn, $sqlSelect_uncofirmed);
                                    $unconfirmed = mysqli_num_rows($result_uncofirmed);
                                    if (mysqli_num_rows($result_uncofirmed) > 0)
                                        { 
                                        while ($row = mysqli_fetch_array($result_uncofirmed)){
                                               ...
                                               $sqlSelect_u = "SELECT * FROM skill where Name = '$name_skil' and user_id = 0 and employer_id != 0 order by rating DESC";                   
                                               $result_u = mysqli_query($conn, $sqlSelect_u); 
                                               $u = mysqli_num_rows($result_u);
                                               $i = 0;
                                               if (mysqli_num_rows($result_u) > 0){ 
